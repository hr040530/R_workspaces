...
# 데이터 수집
# 웹 크롤링(웹페이지 데이터를 긁어내다(가져오다))
# 네이버뉴스기사를 가져와서 데이터프레임으로 생성 후 csv파일로 변환
...
response=requests.get(url,headers=headers) # 요청해서 데이터 get
soup=BeautifulSoup(response.text, 'html.parser') # parser(번역) 해줘!

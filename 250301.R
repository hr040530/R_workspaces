...
setwd('D:/r-data');# D드라이브에 r-data폴더로 경로 수정
...
#형변환 연습
data=data.frame(
  product_number=c('A002','A002','A003'),
...
str('화장품.csv')
# 1. '$' 기호 없애기
data$price=gsub('\\$',"",data$price) # gsub : 특정문자 제거
print(data)
# 2. 문자->숫자로 변환
...
total_sum=colSums(emp[,c("SAL","COMM")],na.rm=TRUE) # na.rm=TRUE : 결측값을 제거하겠다
print(total_sum)
# *****결측값 : 관측/측정에 실패한 값
...
# 중앙값, 표준편차
# 표준편차:데이터가 평균으로부터 얼마나 퍼져 있는지를 나타내는 통계적 지표(흩어진 정도)
# 예) 한 반 수학점수 평균이 78~82가 나왔어요.
# 평균점수가 80이라고 가정할 때 표준편차가 작습니다.
...
# 분위수 : 데이터를 크기 순으로 정렬 후 특정 비율에 해당하는 값
...
# 결측치를 제거한 급여 평균 구하기
sal_mean=mean(emp$SAL,na.rm=TRUE)
print(sal_mean)
# 결측치를 제거한 급여 총합 구하기
sal_sum=sum(emp$SAL,na.rm=TRUE)
print(sal_sum)
# 결측치를 제거한 COMM 표준편차 구하기
comm_sd=sd(emp$COMM,na.rm=TRUE)
print(comm_sd)
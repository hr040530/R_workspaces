...
# 우리는 상자에서 10개만 뽑아서 맛있는 사과의 비율을 추정하려고 합니다.

# 10개를 뽑았을 때, 맛있는 사과의 비율이 항상 80%일까?
# 표본을 무작위로 뽑기 때문에 어떤 겨우에는 7개가 맛있고,
...
# 약 +- 12.56% 범위내에서 변동할 가능성
# 대략 67.35%~92.65% 사이에 있을 가능성이 높다.

# 신뢰구간
# 추가로, 신뢰구간을 계산하면 더 명확하게 범위를 측정할 수 있다.
# 신뢰구간?
# 날씨앱에서 "내일 비가 올 확률이 70%야" 라고 알려줌
# 70%+-5%라고 구간을 알려줍니다
# 신뢰구간은 추측할 값이 실제로 있을 법한 범위를 알려준다
...
# 우리가 뽑은 표본의 결과를 바탕으로 
# 모집단의 맛있는 사과 비율은 약 55%~104% 사이에 있을 가능성이 있음

...
#install.packages('dplyr')# 디플라이어 설치
...
# dplyr : data frame plier(공구) 데이터프레임을 다루는 공구
...
print(high_salary)
# 급여가 3000이상이고 부서번호가 20번인 사원조회
salary3000=emp%>%filter(SAL>=3000&DEPTNO==20)
print(salary3000)
#부서번호가 20번이고 직책은 매니저인 사원 조회
salary20=emp%>%filter(DEPTNO==20&JOB=='MANAGER')
print(salary20)
# 급여가 2000미만이고 부서번호가 20번인 사원 조회
low_salary=emp%>%filter(SAL<2000&DEPTNO==20)
print(low_salary) # SMITH, ADAMS

# 위 문제에서 사원이름, 부서번호만 조회
low_salary=emp%>%filter(SAL<2000&DEPTNO==20)%>%select(ENAME,DEPTNO)
print(low_salary)

# 직책(JOB)이 salesman인 사원의 이름, 직책, 입사날짜 조회
...
emp_comm=emp%>%mutate(TOTAL_COM=SAL+ifelse(is.na(COMM),0,COMM)) # is.na : is==(?) na값이야?
...
manager=emp%>%filter(JOB=='MANAGER')%>%select(DEPTNO,SAL)%>%arrange(SAL)
print(manager)
...
salesman1500=emp%>%filter(JOB=='SALESMAN'&SAL>=1500)%>%select(ENAME,SAL,DEPTNO)%>%arrange(desc(SAL))
print(salesman1500)
#문제 8. 부서 번호(DEPTNO)가 10 또는 30인 직원 중, 이름(ENAME), 직업(JOB), 급여(SAL)을 선택하고 이름 순으로 정렬하세요.
deptno1030=emp%>%filter(DEPTNO==10|DEPTNO==30)%>%select(ENAME,JOB,SAL)%>%arrange(ENAME)
print(deptno1030)
#문제 9. mutate()를 사용하여 급여(SAL)에 보너스(COMM, NA는 0으로 간주)를 더한 총 급여(Total_Salary) 열을 추가하세요.
total_salary=emp%>%mutate(Total_Salary=SAL+ifelse(is.na(COMM),0,COMM))
#View(total_salary)

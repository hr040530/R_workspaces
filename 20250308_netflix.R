...
result=netflix[,c(title,type,release_year)]
print(result)
#문제 4: 데이터셋에서 title, country, rating 열만 선택하세요.
result=netflix[,c(title,country,rating)]
print(result)

# Filter
#문제 5: 2021년에 출시된 영화만 필터링하세요.
result=netflix%>%filter(as.Date(release_year,'%Y년')=='2021년')
print(result)
...
result=netflix%>%group_by(type)%>%summarize(total_count=sum(type),avgerage_release_year=mean(release_year))%>%

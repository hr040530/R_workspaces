...
# test_size=0.2 : 데이터 상위 20% 테스트 데이트로 사용하겠다. 나머지 80% 훈련 데이터
# random_state=42 : 시드값 고정, 매번 실행할 때 동일한 순서로 동일한 값이 테스트 데이터에 포합됩니다.

print('훈련 테스트 : ',x_train)
print('테스트 데이터 : ',x_test)
# 8(훈련데이터):2(테스트 데이터) 일반적인 머신러닝 실험에서 모델의 성능을 평가하기 위한 데이터 분할 비율
...
# x데이터와(리뷰)와 y데이터(라벨링 긍정인지 부정인지)의 정확도를 판단
